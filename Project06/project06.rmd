...
class Node:
    ...
    def to_newick(self):
        ...
        Convert tree to Newick format:
          (A:lengthA,B:lengthB,(C:lengthC,D:lengthD):lengthmiddle) 
        """
        if self.is_leaf():
          return f"{self.name}:{self.dist_left if self.dist_left else 0}"
        
        left_str = self.left.to_newick()
        right_str = self.right.to_newick()
        
        newick = f"({left_str},{right_str})"
        if self.dist_left is not None:  # Add branch length from node to its parent
          newick += f":{self.dist_left}"
...
def build_similarity_matrix(sequences: Dict[str, str]) -> Tuple[np.ndarray, List[str]]:
    ...
    for i in range(n_seq):
        for j in range(i, n_seq):
            ...
            dm [j, i] = dm[i, j]
    #return dm, seq_ids 
    return dm  # return only the matrix only because it doesn't seem like seq_ids is being used
...
def normalize_similarity_matrix(similarity_matrix: np.ndarray) -> np.ndarray:
    ...
    """
    # broke it down for readibility 
    diag = np.diag(similarity_matrix)
    normalization = np.sqrt(np.outer(diag, diag))
    return similarity_matrix / normalization  # was originally returning tuple instead of just np.array
...
def neighbor_joining(
    ...
    """
    n_taxa = distance_matrix.shape[0]
    active_nodes = list(range(n_taxa))  # keep track of remaining active nodes 
    
    # Initialize nodes
    nodes = [Node(name=label) for label in labels]
    
    # keep merging until only 2 nodes remain
    while len(active_nodes) > 2:  
      # build q-matrix 
      q_matrix = build_q_matrix(distance_matrix)
      min_distance, min_coords = get_min_distance(q_matrix)
<<<<<<< HEAD
      i_idx, j_idx = min_coords
      
      # get node indices in our nodes
      i = active_indicies[i_idx]
      j = active_indicies[j_idx]
      
      # calculate branch length of each node
      L_i, L_j = calculate_branch_length(distance matrix, i_idx, j_idx)
      
      # set branch lengths
      nodes[i].dist_left = L_i
      nodes[j].dist_left = L_j
      
      # create internal node for parents of i and j
      new_node = Node(
      ...
      # update distance matrix: remove rows/columns for i/j; add new for merged nodes
      distance_matrix = update_distance_matrix(distance_matrix, i_idx, j_idx)
