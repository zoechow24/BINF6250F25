...
**Input**
## While working with Nikaela (project 4), I implemented a read_fasta that was a hacked up 
##version of Marcus's read fasta from project 3.
##
## I will put that file in our project06 directory as my_read_fasta.py
##      feel free to mess with it, make it more error tolerant, etc, as I haven't checked
##.     for errors on openning or ???
##.I am adding my_read_fasta.py and the appropriate import
  ...
  Distance matrix D for each of the seq by every other sequence
    Create a Dmatrix that is 20 x 20; 
      for each seq1 in sequences[name] do
        for each seq2 in sequences[name] do
          if (seq1 == seq2) Dmatrix[i,j] = 0
          else 
            Dmatrix[i,j] = smith-waterman(seq1,seq2) ???? my smith-waterman returns a matrix, not a score; would need a score here; and per Marcus this should be NORMALIZED ???
            
            
    if eqn1 = $Q(i,j) = (n-2)d(i,j) - \sum_{k=1}^n d(i,k) - \sum_{k=1}^n d(j,k)$
and eqn2 = Branch_length =  $d_i = \frac{d(i,j) + (r_i - r_j)/(n-2)}{2}$
   $d_j = d(i,j) - d_i$
   where $r_i = \sum_{k=1}^n d(i,k)$
and eqn3 $d(k,x) = \frac{d(i,x) + d(j,x) - d(i,j)}{2}$

  T is the tree.
    will need n leaves (a node with only one edge to rest of graph)
    will need internal_nodes
    will need edges associated with all nodes
    will need methods to create_leaves

  Q is the Qmatrix n x n  numpy array.
    where Q[i,j] = eqn1
              ???distance from I to J or is it the distance from i_k and j_k ????
...
If k is an internal node on our graph:
  ...
  # note:  D(k,j) = D(j,k) for all values of j and k
  # also: There will always be a distance from i to j; though they may not be directly connected.
  # also:  nodes will have an idea of 'degrees' as well as to which other nodes they are connected, and which 
    
  S is the sequences Dictionary
    contains 'name' as key
             'seq' as value = string of the acutal sequence
...
**Iteration**: This is to create all the distances D(x,y) so that the graph can be created.
q = n
While q > 2:
1. Calculate Q-matrix where:
  for i in ... do
...
2. Find pair (i,j) with minimum Q(i,j)
  min ( Q ) find the minimum value of the matrix Q (that isn't 0!!!!!!!)
  find S(i) and S(j) that represent that entry in Q.
